...
Tu tarea es escribir una función que, dada una lista de cajas con sus tamaños, determine si es posible empaquetar todas las 
cajas en una sola de manera que cada caja contenga a otra (que a su vez contenga a otra, y así sucesivamente).
...
En el ejemplo anterior, la caja más pequeña entra en la caja más grande. Por lo tanto, es posible empaquetar todas las cajas 
en una sola. Ahora veamos un caso que no:
...
En el ejemplo anterior, la primer caja cabe en la tercera, y la tercera en la segunda. Por lo tanto, es posible empaquetar todas 
las cajas en una sola.
...
function fitsInOneBox(boxes) {
    let ordenar = boxes.sort((a,b) => a.l - b.l)
    return ordenar.every((element, index, array) => {
      if (index === array.length - 1) return true;
      return element.l < array[index +1].l && element.w < array[index + 1].w && element.h < array[index +1].h
    })
}
...
function fitsInOneBox(boxes) {
    ...
    let con = 1;
    for (let x=0; x < sortBoxes.length; x++) {
        if (x === sortBoxes.length - 1) return true;
        if (sortBoxes[x].l < sortBoxes[con].l && sortBoxes[x].w < sortBoxes[con].w && sortBoxes[x].h < sortBoxes[con].h ){
            con+=1;
        }
        else {
          return false;
